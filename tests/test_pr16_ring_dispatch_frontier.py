"""追加6rootの保存pending結合と復元前source固定。旧採取/contractは実行しない。"""
from pathlib import Path
import sys
import tempfile
import unittest
sys.path.insert(0,str(Path(__file__).resolve().parents[1]/'scripts'))
import pr16_ring_dispatch_frontier as f
...
def analysis():
    return {'pending_direct_callees':list(f.CALLEES),'pending_effective_targets':list(f.EFFECTIVE),
        ...
        'all_live_frames_proven':False,'caller_pointer_size_limit_proven':False,'ring_acquisition_accepted':False,'release_ready':False}


class PlanTests(unittest.TestCase):
    def test_six_exact_roots(self):self.assertEqual(f.plan(analysis()),[0x080017d1,0x080020bd,0x0806dc51,0x0806dc57,0x081c7acd,0x09128221])
    def bad(self,k,v):
        a=analysis();a[k]=v
    ...
    def test_changed_node_count(self):self.bad('saved_node_count',2108)
    def test_lost_specific_caller_proof(self):self.bad('specific_nonnull_return_proven',False)
    def test_no_live_task_promotion(self):self.bad('all_live_slot_bounds_proven',True)
    def test_no_live_buffer_promotion(self):self.bad('all_dispatch_returns_proven',True)
    ...
    def test_no_live_frame_promotion(self):self.bad('all_live_frames_proven',True)
    def test_no_allocation_promotion(self):self.bad('caller_pointer_size_limit_proven',True)
    def test_requires_caller_allocation(self):self.bad('slot_safety_requires_caller_allocation',False)
    def test_bad_analysis_type(self):
        with self.assertRaises(ValueError):f.plan([])
    def test_bad_run_type(self):
        with self.assertRaises(ValueError):f.preflight(Path('.'),('source',),analysis(),True)
