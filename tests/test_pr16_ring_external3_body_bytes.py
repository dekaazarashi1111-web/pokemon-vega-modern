"""未読継続の採取scopeと保存前半の契約を検証。候補の復元はmockする。"""
import copy
...
import pr16_ring_external3_body_bytes as m

class External3BodyCollection(unittest.TestCase):
    ...
    def test_wrong_task(self):
        self.prior['task'] = 'wrong'; self.reject()
    def test_wrong_priority(self):
        self.prior['analysis']['priority_unread_targets'][0] = 0x0806DD1D; self.reject()
    ...
    def test_target_must_be_unread(self):
        self.prior['analysis']['remaining_unread_targets'].remove(m.TARGET); self.reject()
    ...
    def test_live_frame_preserved(self):
        self.prior['analysis']['frame_bytes_live'] = 0; self.reject()
    def test_alias_diagnostic_not_observation(self):
        self.prior['analysis']['hypothetical_stack_overwrites_counter']['observed'] = True; self.reject()
    ...
    def test_single_sampler_no_prior_execution(self):
        ...
        self.assertEqual(set(collect.call_args.args[1]), set(self.prior['analysis']['old_unread_targets']) | {0x08113961})
