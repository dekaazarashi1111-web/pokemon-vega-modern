...
import pr16_ring_external3_bytes as m

class External3Collection(unittest.TestCase):
    def setUp(self):
    ...
    def test_local_return_preconditions_preserved(self):
        self.prior['analysis']['external2_local_return_under_model_preconditions'] = False
        with self.assertRaises(ValueError): m.validate_prior(self.prior)

    def test_prior_conditional_boundary(self):
        self.prior['analysis']['external2_unconditional_return_proven'] = True
        with self.assertRaises(ValueError): m.validate_prior(self.prior)
    ...
    def test_single_sampler_no_prior_execution(self):
        ...
        self.assertEqual(result['prior_external_indirect_edges_preserved'], self.prior['analysis']['prior_external1_indirect_edges_preserved'] + self.prior['analysis']['unresolved_indirect_edges'])
