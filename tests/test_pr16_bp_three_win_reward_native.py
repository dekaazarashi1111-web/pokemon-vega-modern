...
class ThreeWinRewardNativeTests(unittest.TestCase):
    ...
    def test_controller_extends_only_unobserved_suffix(self):
        ...
        second_start = text.index('rw_trace(c,"second-start");')
        second_budget = text.index("wx_voluntary_count=0U;", second_start)
        second_call = text.index("br_battle_return(c,original,counter,2U)", second_start)
        self.assertLess(second_start, second_budget)
        self.assertLess(second_budget, second_call)
        self.assertIn("wx_exchange_next(c,party,counter,finish.outcome,1U)", text)
        self.assertIn("rw_exchange_next(c,original,counter,second.outcome)", text)
        self.assertIn("RW_EXCHANGE2_OWNER_SCRIPT 0x092CF778U", text)
        self.assertIn("RW_BATTLE3_SCRIPT 0x092CF6BCU", text)
        self.assertIn("static void rw_single_confirm(struct mCore *c,unsigned slot)", text)
        self.assertIn("rw_commit_returned(c)", text)
        self.assertIn("second native exchange did not reach third battle/action", text)
        self.assertIn("static unsigned rw_cleanup_reserve(struct mCore *c,unsigned active)", text)
        self.assertIn("unsigned strongest=wx_reserve(c,active),target=strongest,living=0U;", text)
        self.assertIn("if(i!=strongest)target=i;", text)
        self.assertIn("static void rw_switch(struct mCore *c,struct BPReturn *w)", text)
        self.assertIn("rw_switch(c,&w);continue;", text)
        third_start = text.index('rw_trace(c,"third-start");')
        third_budget = text.rindex("wx_voluntary_count=0U;", 0, third_start)
        third_action = text.index("if(!wx_voluntary_count){wx_opening_switch(c);continue;}", third_start)
        self.assertLess(third_budget, third_start)
        self.assertLess(third_start, third_action)
    ...
    def test_new_controller_source_has_no_game_memory_write_or_direct_call(self):
        ...
        self.assertIn("rw_switch(c,&w)", source)
