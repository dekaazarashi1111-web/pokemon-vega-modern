"""未読外部calleeの採取scopeと保存境界を検証。候補の復元はmockする。"""
import copy
...
import pr16_ring_external2_bytes as m

class External2Collection(unittest.TestCase):
    def setUp(self):
        self.prior = m.s.load(m.PRIOR)
    ...
    def test_wrong_priority(self):
        self.prior['analysis']['priority_unread_targets'].reverse()
        with self.assertRaises(ValueError): m.validate_prior(self.prior)
    ...
    def test_other_callee_preserved(self):
        self.prior['analysis']['remaining_unread_targets'].remove(0x081138F9)
        with self.assertRaises(ValueError): m.validate_prior(self.prior)
    ...
    def test_alias_diagnostic_not_observation(self):
        self.prior['analysis']['hypothetical_counter_corrupts_saved_lr']['observed'] = True
        with self.assertRaises(ValueError): m.validate_prior(self.prior)

    def test_prior_conditional_boundary(self):
        self.prior['analysis']['composition']['unconditional_external1_return_proven'] = True
        with self.assertRaises(ValueError): m.validate_prior(self.prior)
    ...
    def test_saved_edge_not_reopened(self):
        ...
        self.assertEqual(r['known_sampled_boundary_targets'], [target])
        self.assertNotIn(target, r['remaining_unread_targets'])
    ...
    def test_single_sampler_no_prior_execution(self):
        ...
        self.assertEqual(collect.call_args.args[0], m.TARGET)
        self.assertIn(0x081138F9, collect.call_args.args[1])
        self.assertIn(m.previous.REPORT, collect.call_args.args[2])
        self.assertEqual(result['candidate_reconstructions'], 1)
        self.assertEqual(result['prior_abi_classifications_replayed'], 0)
        self.assertFalse(result['side_effects_excluded'])
        self.assertEqual(result['prior_external1_indirect_edges_preserved'], self.prior['analysis']['unresolved_indirect_edges'])
