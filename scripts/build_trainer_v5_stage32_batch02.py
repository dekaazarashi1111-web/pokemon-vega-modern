...
RUNNER = Path("tools/mgba_trainer_v5_stage32_batch02_smoke.c")
SOURCE_DIR = Path("content/trainer_v5_stage32_batch02")

EXPECTED_INPUT_SHA256 = "629e494de9ee34e5de7b87041c3e77ca163e658e27e93b3666abb6e5a729bed3"
...
REQUIRED_ENTRYPOINTS = {
    ...
    "TrainerV5Runtime_ExactRebindTable",
}
AI_FLAGS = {"AI_BASIC": 1, "AI_SEMI_SMART": 3, "AI_FULL_SMART": 5}
SINGLE_KINDS = {0, 3, 5, 9}
DOUBLE_KINDS = {4, 7}
...
def _input_contract(root: Path) -> tuple[bytes, bytes, dict[str, Any], dict[str, Any], dict[str, Any], dict[str, Any]]:
    ...
    if (trainer.get("record_size") != TRAINER_RECORD_SIZE
        ...
        _fail("Stage 17 trainer template ABI metadata differs")
...
def load_and_validate_source(root: Path = ROOT, stage: bytes | None = None) -> dict[str, Any]:
    ...
    schema = _read_json(source / "trainer_v5_stage32.schema.json")
    x_contract = schema.get("x-stage32-contract", {})
    if (x_contract.get("single_kinds") != [0, 3, 5, 9]
        ...
        _fail("Trainer V5 Stage32 schema contract differs")
    ...
    for domain, key, subset_name, live_name in (
        ...
        live = _index(_rows(root / live_name), key, f"Stage32 {domain} registry")
        runtime: dict[str, int] = {}
        for name, original in subset.items():
            if name not in live:
                _fail(f"V5 {domain} key missing from Stage32 registry: {name}")
            ...
            if source_id != runtime_id:
                id_rebindings.append({
                    ...
                    "stage32_id": runtime_id, "reason": "key-based canonical Stage32 resolution",
                })
...
def _resolve_ability_mode(stage: bytes, species_meta: Mapping[str, Any], species_id: int, ability_id: int) -> int:
    ...
    if not matches:
        _fail(f"species {species_id}: requested ability {ability_id} absent from Stage32 base stats {abilities}")
    return matches[0]
...
def _serialize_catalog(source: Mapping[str, Any], stage: bytes, species_meta: Mapping[str, Any]) -> dict[str, Any]:
    ...
    exact_rebinds = sorted(({
        "data_address": int(row["script_instruction_address"], 0) + 1,
        "kind": int(row["trainerbattle_kind"], 0),
        "source_trainer_id": int(row["source_trainer_id"], 0),
...
def _generated_header(serialized: Mapping[str, Any]) -> bytes:
    ...
    exact_lines = [
        ...
        for row in serialized["exact_rebinds"]
    ]
    text = f"""#ifndef VEGA_TRAINER_V5_STAGE32_GENERATED_H
#define VEGA_TRAINER_V5_STAGE32_GENERATED_H
...
#define TRAINER_V5_GENERATED_EXACT_REBIND_COUNT {len(serialized['exact_rebinds'])}u
...
static const struct TrainerV5ExactRebindV1 gTrainerV5ExactRebinds[TRAINER_V5_GENERATED_EXACT_REBIND_COUNT] = {{
...
def _compile_runtime(
    ...
    source = root / "overlays/trainer_v5_stage32_batch02_runtime/trainer_v5_stage32_runtime.c"
    ...
    with tempfile.TemporaryDirectory(prefix="vega-trainer-v5-runtime-") as temporary:
        ...
        (directory / "trainer_v5_stage32_generated.h").write_bytes(header)
...
def build_runtime_outputs(root: Path = ROOT) -> dict[str, bytes]:
    ...
    source_template_table = int(trainer_meta["trainers"]["address"])
    previous_table = int(input_meta["trainer_table"]["new_address"])
    new_table = int(runtime["payload"]["trainer_table_address"])
    repoints: list[dict[str, Any]] = []
    previous_repoints = {
        int(row["site_offset"]): row
    ...
    previous_hooks = {row["name"]: row for row in input_meta["hooks"]}
    hook_rows: list[dict[str, Any]] = []
    for name in HOOK_NAMES:
        hook = HOOKS[name]
        previous = previous_hooks[name]
        ...
        hook_rows.append({
            ...
            "expected_hex": expected.hex(), "stock_prologue_hex": hook["expected"].hex(),
    ...
    metadata: dict[str, Any] = {
        ...
        **runtime,
        ...
        "trainer_table": {
            "old_address": previous_table, "old_count": TRAINER_TABLE_COUNT,
            "source_template_address": source_template_table, "source_template_count": OLD_TRAINER_TABLE_COUNT,
        ...
        "runtime_id_rebindings": serialized["exact_rebinds"],
        "allocation": {
...
def _mgba_fixture(root: Path, stage: bytes, metadata: Mapping[str, Any]) -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="vega-trainer-v5-smoke-") as temporary:
        ...
        common_args = [
            ...
            hex(entry["TrainerV5Runtime_ConfigureTrainerBattle"]),
