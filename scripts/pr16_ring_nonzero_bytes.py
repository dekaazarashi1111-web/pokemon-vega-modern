#!/usr/bin/env python3
"""helper非0側の未読14byte境界だけを採取し、ABI検証用に保存する。"""
...
BASE = 'ef5a6e92bd91c7ba3aa36e1ee66eec2e5bb695b2'
TASK = 'PR-P08-7-RING-NONZERO-BYTES'
SELF = 'scripts/pr16_ring_nonzero_bytes.py'
TEST = 'tests/test_pr16_ring_nonzero_bytes.py'
WORKFLOW = '.github/workflows/pr16-ring-nonzero-bytes.yml'
PRIOR = 'content/modernization/pr16_ring_helper_abi.json'
REPORT = 'content/modernization/pr16_ring_nonzero_bytes.json'
...
OUT = ROOT / '.local/pr16-ring-nonzero-bytes'
TARGET, WINDOW, LIMIT, ROM_BASE = 0x090970F7, 14, 7, 0x08000000
GAP = 'P05_NATIVE_RING_ACQUISITION_PHYSICAL'
SOURCES = (SELF, TEST, WORKFLOW, RESTORE, PRIOR,
    ...
    'content/modernization/pr16_ring_helper_bytes.json',
...
def validate_graph(graph, cached):
    """既読callee先頭0x09097104を含めず、領域外を追わない。"""
    root, end = TARGET & ~1, (TARGET & ~1) + WINDOW
    need(graph['entry'] == TARGET and graph['window'] == WINDOW, 'root/window differs')
    ...
    need(starts == sorted(set(starts)) and starts[0] == root, 'node order/root differs')
    occupied = set()
    for n in nodes:
        at, size = n['address'], n['size']
        need(type(at) is int and not at & 1 and size in (2, 4), 'invalid instruction')
        need(root <= at and at + size <= end, 'escaped unread range')
        span = set(range(at, at + size))
        need(not span & (occupied | cached), 'overlap/previous code rescan')
        occupied |= span
        need(len(bytes.fromhex(n['hex'])) == size, 'instruction size differs')
...
def inputs():
    ...
    need(a['helper_return_proven'] is True and a['helper_return_observed'] is False, 'helper boundary differs')
    need(a['callee_return_proven'] is False and a['old_frontier_removed'] is False, 'overclaim')
    need(len(a['old_unread_targets']) == 18 and 0x0806DDBD in a['additional_unread_targets'], 'frontier differs')
    _, cached = previous.read_prior()
    for path in ('content/modernization/pr16_ring_callee_bytes.json',
                 'content/modernization/pr16_ring_helper_bytes.json'):
        value = json.loads((ROOT / path).read_bytes())
        for n in value['analysis']['graph']['nodes']:
            cached.update(range(n['address'], n['address'] + n['size']))
    need(not set(range(TARGET & ~1, (TARGET & ~1) + WINDOW)) & cached, 'window already read')
    return prior, cached
...
def preflight():
    ...
    active = [r for r in runs if r['status'] != 'completed' and r['id'] != int(os.environ['GITHUB_RUN_ID'])
              and '/pr16-ring-' in r['path']]
    need(not active, 'another Ring run active')
...
def record(rom):
    ...
    graph = decoder.native_graph(raw, TARGET, window=WINDOW, limit=LIMIT)
    ...
    a = prior['analysis']
    result = {'classification': 'NONZERO_CONTINUATION_BYTES_NOT_ABI_PROOF',
        ...
        'priority_unread_targets': [TARGET], 'helper_return_proven': True,
    ...
    suite = unittest.defaultTestLoader.discover('tests', pattern='test_pr16_ring_nonzero_bytes.py')
    ...
    next(r for r in backlog['remaining_conditions'] if r['id'] == 'NATURAL_CAPTURE_GEAR')['ring_nonzero_bytes'] = REPORT
    stop = (f'WIP: 未読0x090970F7だけを最大14byte範囲で{len(graph["nodes"])}命令/'
        f'{result["sampled_instruction_bytes"]}byte採取し保存。helper非0側の返却pointer/SP/r4-r6/保存slotは保存byteで検証する。'
        '旧18target・未読0x0806DDBD・BP受入を保持。')
    next_step = ('保存済みpr16_ring_nonzero_bytes.jsonの0x090970F7継続だけを限定ABI検証する。'
        '候補復元/byte採取/helper全u16/既読callee/FlagSet/FlagGet/15辺分類/BPを再実行しない。'
        '0x0806DDBDは未読。callee全体・全caller/全owner除外・Ring通常取得へ昇格しない。')
    state['ring_nonzero_bytes'] = {'path': REPORT, 'run_id': value['run_id'], 'source_head': before['head'], 'target': TARGET}
    state['bp']['current_stop'] = state['source_change_review_ja'] = stop
    state['bp']['next_step'] = state['next_action']['goal_ja'] = next_step
    state['next_action']['read_paths'] = [REPORT, SELF, PRIOR, SOURCES[5], SOURCES[6]]
    state['observed_head'] = before['head']
    state['observed_head_semantics'] = '未読非0継続一根の採取source HEAD。保存commit/runはremote ref/Actionsで確認。'
    state['observed_head_checks']['reason_ja'] = f'helper run{prior["run_id"]}とBP run34946969126成功照合。今回run{value["run_id"]}は保存時in_progress。action_requiredは成功へ読み替えない。'
    state['session_execution_summary'] = {'new_emulator_processes': 0, 'rom_changes': 0,
        ...
        'scope_ja': '未読非0継続一根だけ採取。既読graph/受入済みnative再実行0。'}
    state['do_not_repeat'].append('0x090970F7の限定byte採取は保存済み。保存byteを再利用し、同一candidateを再構築/再採取しない。')
...
if __name__ == '__main__':
    if sys.argv[1:] == ['preflight']:
        ...
        raise SystemExit('usage: pr16_ring_nonzero_bytes.py preflight|restore|record <candidate>')
