#!/usr/bin/env python3
"""Stage83 remaining-route native input probes; never grants whole-phase acceptance."""
...
SCOPE='P03_FORGET_NATIVE_INPUT_COLD_SAVE_STAGE83'
...
def run(out,jobs=2,selected=None):
    ...
    rows=cases();names=[c['name'] for c in rows]
    if selected is not None:
        need(selected in names,'unknown case');rows=[c for c in rows if c['name']==selected]
    ...
    try:
        with tempfile.TemporaryDirectory(prefix='remaining-routes-',dir=ROOT/'.local') as td:
            ...
            def one(item):
                ...
                raw,err,p=base.capture([str(binary),str(rom),str(save),ROM_SHA,SEED_SHA,str(i)],out/c['name'],240)
                result=validate(raw,c,p,err)
                return dict(name=c['name'],process=p,result=result,stdout=identity(out/(c['name']+'.stdout')),stderr=identity(out/(c['name']+'.stderr')))
            with ThreadPoolExecutor(max_workers=jobs) as pool:results=list(pool.map(one,enumerate(rows)))
    finally:
    ...
    report=dict(schema_version=1,status='PASS',scope=SCOPE,candidate_stage=83,rom=rid,seed=sid,source_bindings=binding,
                cases=results,guard_checks=list(GUARDS),fresh_processes=len(results),core_instances=2*len(results),cache_reuse=0,
