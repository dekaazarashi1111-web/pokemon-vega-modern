#!/usr/bin/env python3
"""BIOS限定工程の記録。既存followup v2のゲートを維持し一次資料をログへ明記する。"""
...
SELF = 'scripts/pr16_ring_bios_record.py'
...
def assert_remote(head, attempts=1):
    """remote refを毎回確認。push後のPR API反映遅延だけを有限再読する。"""
    for attempt in range(attempts):
        ...
        need(pr['state'] == 'open' and pr['draft'] and not pr['merged']
             and pr['head']['ref'] == BRANCH and pr['head']['repo']['full_name'] == REPO, 'PR境界不一致')
        if pr['head']['sha'] == head:
            return pr
        if attempt + 1 < attempts:
            time.sleep(1)
    raise ValueError('PR HEAD反映を確認できない')
...
def run(task):
    ...
    for rid, sha in ((prior['run_id'], prior['source_head']),
        ...
        need(r['status'] == 'completed' and r['head_sha'] == sha, '先行Actions不一致')
        observed = {k: r[k] for k in ('id', 'head_sha', 'status', 'conclusion')}
        if r['conclusion'] != 'success':
            need(rid == prior['run_id'] and hasattr(task, 'recover_prior'), '先行Actions失敗')
            observed['scoped_recovery'] = task.recover_prior(prior, r)
            need(observed['scoped_recovery']['record_verified'] is True, '保存済み記録の独立検証失敗')
    ...
    value = {'schema_version': 1, 'task': task.TASK, 'source_head': head,
        ...
        'prior_actions_verified_without_replay': actions,
    ...
    state['observed_head_checks']['reason_ja'] = f'先行run{prior["run_id"]}の原結論と保存証拠、BP run34946969126成功を照合。今回run{value["run_id"]}は記録時in_progress。action_requiredを成功へ読み替えない。'
    ...
    assert_remote(commit, attempts=12)
    for p in outputs:
