...
def remember_originals(files,run):
    ...
    return {directory+'/'+name:identity(raw) for name,raw in files.items()}
...
def record():
    ...
    unit=subprocess.run([sys.executable,'-B','-m','unittest','discover','-s','tests','-p','test_pr16_research_save_impact.py','-v'],cwd=ROOT,capture_output=True)
    OUT.mkdir(parents=True,exist_ok=True);(OUT/'new-unit.txt').write_bytes(unit.stdout+unit.stderr)
    need(unit.returncode==0 and (unit.stdout+unit.stderr).count(b' ... ok\n')==68 and b'\nOK\n' in unit.stderr,'68 focused unit tests')
    ...
    cp={'schema_version':1,'task':TASK,'status':'PASS_RESEARCH_SAVE_IMPACT_18_SCOPED','scope':'SCHEDULED_PRODUCTION_TRANSACTION_SERVICES_NOT_NORMAL_TRANSACTION_UI',
        ...
        'new_unit_tests':68,'excluded_scopes':['normal transaction UI','all catalog rows','new save / V1 migration','phase0 failure injection','map3/19 excluded130 rows'],
    ...
    receipt={'mode':'record','source_head':os.environ['GITHUB_SHA'],'run_id':int(os.environ['GITHUB_RUN_ID']),'new_native_processes':0,'host_compiles':0,'arm_compiles':0,'accepted_case_reruns':0,
             'new_unit_tests':68,'native_runs':runs,'artifacts':artifacts,'pre_native_failure':run_summary(pre),'evidence_manifest':cp['evidence_manifest'],
...
def finalize():
    ...
    need(old['new_native_processes']==old['host_compiles']==old['arm_compiles']==0 and old['new_unit_tests']==68,'original record scope')
    ...
    receipt={'mode':'finalize','source_head':os.environ['GITHUB_SHA'],'run_id':int(os.environ['GITHUB_RUN_ID']),'new_native_processes':0,'host_compiles':0,'arm_compiles':0,'new_unit_tests':0,'accepted_case_reruns':0,
...
def publish(cp,receipt,mode):
    ...
    s['observed_head_checks']={'scope_head':m.ITEM_SOURCE,'runs':[{'id':36229762846,'head_sha':m.ITEM_SOURCE,'status':'completed','conclusion':'success'}],
        'previous_failed_runs':[{'id':36228964902,'conclusion':'failure'},{'id':36229142708,'conclusion':'failure','accepted_earn_cases':8,'failed_item_cases_retained':8}],
