...
OUTPUTS=(REPORT,TRACE,s.STATE,s.DOC,s.BACKLOG,*s.LOGS)
...
def preflight():
    ...
    s.need(tests.wasSuccessful() and not tests.skipped and tests.testsRun>=42,'focused tests')
...
def record():
    ...
    state['session_execution_summary']={'new_emulator_processes':1,'fresh_cores':1,
        'rom_changes':0,'candidate_reconstructions':1,'accepted_standalone_replays':0,
        'scope_ja':stop}
    ...
    for p in (*CODE,REPORT,TRACE):state['source_bindings'][p]=s.identity((s.ROOT/p).read_bytes())
    ...
    entry=(f'\n\n## {stamp} — {TASK}\n- Timestamp: {stamp}\n- Task: {TASK}\n'
        ...
        '- Preserved: ROM変更0、受入済みnative/既読ABI再実行0、正式BP checkpoint不変。新規boot診断1 process/1 core、候補復元1。\n'
    ...
    receipt={'status':'PASS_RECORDED_NONFORCE_PUSHED','commit':commit,'source_head':head,
        ...
        'bound_calls':result['bound_calls'],'new_emulator_processes':1,'accepted_native_cases_replayed':0,
