#!/usr/bin/env python3
"""T14 full Kanto map import build/check."""
...
def main() -> int:
    ...
    try:
        ...
        if args.mode == "build":
            ...
            print(f"T14 build: PASS ({len(outputs)} artifacts)")
        else:
            drift = [relative for relative, raw in outputs.items()
                     if not (ROOT / relative).is_file() or (ROOT / relative).read_bytes() != raw]
            if drift:
                raise BuildError("artifact drift: " + ", ".join(drift[:8]))
            print(f"T14 check: PASS ({len(outputs)} artifacts, side effects NONE)")
    except (BuildError, OSError, ValueError, KeyError) as exc:
        print(f"T14 {args.mode}: FAIL: {exc}", file=sys.stderr)
        return 1
    return 0
