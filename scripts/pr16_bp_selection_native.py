#!/usr/bin/env python3
"""Observe native second confirmation and battle action on unchanged bffd; NOT BP acceptance."""
...
CASE='second-confirm-battle-probe'
STATUS='DIAGNOSTIC_SECOND_CONFIRM_BATTLE_ACTION_NOT_BP'
...
def validate(raw,stderr,code):
    ...
    need(0<row['selected_frame']<row['second_chooser_frame']<row['confirm_frame']<=row['script_advance_frame']
...
def run():
    ...
    report=dict(schema_version=1,status='FAIL',scope='SECOND_CONFIRM_DIAGNOSTIC_NOT_EARNING',candidate=dict(size=33554432,sha256=SHA),
