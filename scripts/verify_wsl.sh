...
# Minimal verify script for WSL/Linux projects.
# Customize the checks below to match your project (pytest, npm test, make verify, etc).
...
for path in root.rglob("*"):
    if path.is_dir():
        if path.name in ignore:
            # skip entire ignored directory subtree
...
if [ $ran_check -eq 0 ]; then
  ...
  echo "[verify] Edit scripts/verify_wsl.sh to add pytest/npm/make checks for this repo."
fi
