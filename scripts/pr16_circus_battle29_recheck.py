#!/usr/bin/env python3
"""28勝prefixと29戦目初手維持を保持し、HP閾値再評価後だけを1回進める。"""
...
TASK='USER-20260920-CIRCUS-BATTLE29-RECHECK'
REPORT='content/modernization/pr16_circus_battle29_recheck.json'
OLD_REPORT='content/modernization/pr16_circus_battle29.json'
OLD_PREFIX='evidence/pr16_circus_battle29/35473090294/'
FILES=('scripts/pr16_circus_battle29_recheck.py','scripts/pr16_circus_battle29_recheck_policy.py',
       'tests/test_pr16_circus_battle29_recheck.py','.github/workflows/pr16-circus-battle29-recheck.yml')
RUN=35473090294
JOB=105977531675
ART=10593187955
WORKFLOW_HEAD='e125a2d4b3c0cf7e012eba9fb14b079530acaa18'
ARCHIVE=dict(size=1051212,sha256='2ec5a7475dbf7cae02f4638942c502acad3ffeaaf724de9e633da45ac741d016')
ROOT=b.ROOT
b.REPORT=REPORT
b.TASK=TASK
b.policy=SimpleNamespace(adapt=p.adapt,prefix_proof=p.parent.parent.previous.prefix_proof)
need=b.need
...
def previous_attempt():
    ...
    need(old['recording_run']==RUN and old['source_head']==WORKFLOW_HEAD
         and old['new_emulator_processes']==1 and old['lifecycle_verified']
         and old['candidate']==b.TARGET and not old['failures']
         and old['classification']=='CIRCUS_BATTLE29_LOSS_SAVE_SCOPED_30_OPEN'
         and old['result']['wins']==28 and old['result']['losses']==1
         and old['prefix29']['exact_event_count']==134
         and old['prefix29']['continuation_prefix_wins']==28,'previous scoped battle29 acceptance')
    ...
    need(artifact['workflow_run']['id']==RUN and artifact['size_in_bytes']==ARCHIVE['size']
         and artifact['digest']=='sha256:'+ARCHIVE['sha256'],'previous artifact scope')
    return dict(run_id=RUN,job_id=JOB,artifact_id=ART,archive=ARCHIVE,workflow_source_head=WORKFLOW_HEAD,
                ...
                reason_ja='29戦目初手の水先発維持と134eventsを保持し、同じ相手を処理済みにせずHP半分まで再評価する。')
...
def summary(value,phase):
    if value.get('genuine_30_wins_verified'):
        return ('同一2b107e7eで真正30勝90BP・元party600/owner64・通常Save/fresh Continueを検証。'
                '受入28勝＋29戦目actionまで134events、初手維持byte列、HP半分での通常交代復帰を固定照合。ROM変更/ARM0。',
                'このrun原本と終端画面を照合し、正規実受付から特性抑制へ進む。30勝単独再実行は禁止。P08移送・releaseは未完。')
    if value.get('lifecycle_verified'):
        r=value['result']
        return (f"HP閾値再評価後、実{r['wins']}勝/{r['bp_earned']}BPと通常Save/fresh Continueをscoped検証。真正30勝は未達。",
                f"{REPORT}の新たな最初の実敗北だけを修復する。受入単体・旧ARM・Ring hostを再実行しない。")
    if phase=='START':
        return ('28勝81BPと通常Save/fresh Continue、29戦目初手維持を保持。健康な水先発を維持した相手を処理済みにせず、'
                'HP半分まで毎turn再評価して既存の通常交代へ戻す修正と新規9契約を検証。新nativeは未実行。',
                '進行中の29戦目再評価Actionsだけを照合する。重複起動せず、完了後に原本・固定引継ぎ・両ログを記録する。')
    d=value.get('partial_diagnostic',{})
    return (f"29戦目HP閾値再評価の原本を保存。観測勝数{d.get('settled_wins','未採取')}。Save/Continue受入とは区別。"+str(value.get('failures',[])),
            f'{REPORT}の失敗段階と134event/再評価prefix証拠を読んで未完段階のみ修復。同じ失敗/旧builder/ARM/Ring host/受入単体を再実行しない。')
...
def checkpoint(value,phase):
    ...
    state['circus_battle29_recheck']=dict(path=REPORT,run_id=run_id,classification=value['classification'],candidate=b.TARGET)
    state['bp']['current_stop']=state['source_change_review_ja']=stop
    state['bp']['next_step']=state['next_action']['goal_ja']=nxt
    state['next_action']['id']='CIRCUS_SUPPRESSION_NATIVE' if value.get('genuine_30_wins_verified') else 'CIRCUS_BATTLE29_RECHECK'
    ...
    state['session_execution_summary']['scope_ja']='当runの実起動数のみ。保存byte/固定生成Cを再利用。28勝は次の未完部分への連続prefixで、受入単体は再実行しない。歴史的ARM総数unknownは維持。'
    ...
    state['observed_head_checks']=dict(scope_head=head,runs=[dict(id=RUN,head_sha=WORKFLOW_HEAD,status='completed',conclusion='failure')],
        reason_ja='前runの28勝/Save成功/29戦目実敗北を保持。今回runは記録時in_progress。全CI greenは主張しない。')
    state['pending_runs']=[dict(run_id=run_id,tested_head=value['source_head'],status='in_progress',scope='battle29-hp-recheck')]
    note=f'{TASK} run{run_id}の新原本を先に読む。旧run{RUN}の28勝/29戦目実敗北を30勝へ改作しない。受入単体/旧ARM/Ring host再実行は禁止。'
    ...
    row['battle29_recheck_checkpoint']=REPORT;row['resume']=nxt
    if value.get('lifecycle_verified'):row['implementation_checkpoint']=REPORT;row['implementation_status']=value['classification']
    evidence={};prefix='evidence/pr16_circus_battle29_recheck/'+str(run_id)+'/'
    ...
    subprocess.run(['git','commit','-m',TASK+': '+phase+' HP閾値再評価・原本・固定引継ぎ・両ログを同期'],cwd=ROOT,check=True)
...
def prepare():
    ...
    source_guard.BASE='f52dc173675e56e8b9d59fa030695293a5b66a40'
    source_guard.OUT=b.OUT;source_guard.ALLOWED=set(FILES);source_guard.guard()
    prior=previous_attempt()
    out,err,proc=b.capture([sys.executable,'-m','unittest','discover','-s','tests','-p','test_pr16_circus_battle29_recheck.py','-v'],'recheck-tests')
    need(b.exited(proc)==0 and b'Ran 9 tests' in err and b'\nOK\n' in err,'recheck contracts')
    value=dict(schema_version=1,task=TASK,classification='CIRCUS_BATTLE29_RECHECK_IMPLEMENTED_NATIVE_PENDING',source_head=head,
...
def native():
    ...
    if path.exists():
        raw=path.read_bytes()
        try:
            value['partial_diagnostic']=p.parent.parent.diagnostic(raw)
            value['prefix29_recheck']=p.prefix_proof(old_trace(),raw)
            (b.OUT/'execution/prefix29-recheck.json').write_bytes(b.stable(value['prefix29_recheck']))
        except Exception as error:value['failures'].append(dict(stage='prefix29-recheck',type=type(error).__name__,error=str(error)))
    ...
    value['classification']=('CIRCUS_GENUINE_30_WINS_90BP_SAVE_CONTINUE' if value['genuine_30_wins_verified'] else
                             'CIRCUS_BATTLE29_RECHECK_LOSS_SAVE_SCOPED_30_OPEN' if value['lifecycle_verified'] else 'CIRCUS_BATTLE29_RECHECK_DIAGNOSTIC_OPEN')
