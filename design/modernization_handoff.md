...
- P05のAbility ROM checkpointはStage77まで進んだ。Stage72のAbility 312〜317／29 battle hookとStage76の安全な3 edgeを継承し、Battle Circusの全特性無効時だけ29 hookをStage72 original trampolineへ迂回させた。通常時はStage72 wrapperを維持し、Gastro Acid／Neutralizing Gas／Mold Breakerの既存抑制も変更しない。Stage77 ROM SHA-256は`245133a4740dda9faa0663d321505ee793293d64b0b318d601fd91933b84973f`、CRC32は`F1CE0EAC`。P08 selected candidateはこのStage77へ統合済み。Eelevate専用switch AIと最終累積mGBAは未完了のまま保持する。
...
- P08はStage77までのcheckpoint hash、生成実装、ignored ROM/BPS、evidence sourceを統合監査済み。Stage74→75→76→77のincremental BPSとallocator lineageをfail closedで固定した。active baseline Stage62、P01のみDONE、P02〜P08未完了、release-ready=falseを維持する。
