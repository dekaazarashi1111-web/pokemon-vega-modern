...
- T17 Regression: `make regression` が `build/stages/17_regression.{gba,json}`、中央allocation、QOL-B runtime、Kanto map/wild/trainer/progression実体、トーホク外来生態293行・Item 348、manual checkpointとregression reportを生成する。`make regression-check` は2回のbyte同一build、libmGBA 2 process、RTC／手動mode、通常／釣り／隠れ遭遇、95 entry・294 candidate binding、QOL-B host境界、253 map往復到達、400地方往復、125共有捕獲、施設/AI/event状態fixture、成果のbyte一致を副作用なしで照合する。
...
- USER HM field能力: `make hm-field-access` がstage 21へVega HM Item 339〜346を正本とするruntimeを結合し、`build/stages/22_hm_field_access.{gba,json}`、allocation、libmGBA証跡、`reports/generated/hm_field_access.md` を生成する。`make hm-field-access-check` は8 HM×手持ち3条件、snapshot復元、Surf状態、既存callback、clean ROMからのBPS完全往復、7成果のbyte一致を副作用なしで照合する。
- USER battle rules: `make battle-rules` がstage 22のCFRU source/default・command/hook ownerを監査し、zero-patch `build/stages/23_battle_rules.{gba,json}`、allocation、固定RNGとFactory/RaidのlibmGBA証跡、`reports/generated/battle_rules.md` を生成する。`make battle-rules-check` は入力/source/toolchain hashが同じ検証済みfixtureを再利用し、6成果のbyte一致、単一owner、二重適用なし、BPS完全往復を副作用なしで照合する。
- USER battle UI: `make battle-ui` がstage 23へ固定CFRU source相当の技タイプ・有効度・STAB adapterと戦闘中旧HELP guardを結合し、`build/stages/24_battle_ui.{gba,json}`、allocation、UI/Factory/Raid libmGBA証跡、`reports/generated/battle_ui.md` を生成する。`make battle-ui-check` は5表示区分、Stellar/Tera Blast、double対象、通常HELP/LR両設定のL詳細、field HELP維持、戦闘pointer不変、wild/trainer/Factory/Raid入力復帰、canonical文字列、BPS往復、8成果のbyte一致を副作用なしで照合する。
- USER わざメモリー: `make move-memory` がstage 24へItem 347、1個目バッジ報酬、シオウ/カラスバ共通coreを結合し、`build/stages/25_move_memory.{gba,json}`、allocation、libmGBA証跡、`reports/generated/move_memory.md` を生成する。`make move-memory-check` は通常Lv境界、タマゴ技5条件、最後の1技、PP Up、HM、form連動、context、BPS往復、7成果のbyte一致を副作用なしで照合する。
- USER QOL release: `make qol-release-smoke` がstage 20→25のchainを確認し、同一stage 25でKanto/QOL-B、外来生態、Factory、初戦、HM、戦闘規則/UI、わざメモリーを再観測して `build/stages/25_mgba_qol_release.json` と `reports/generated/qol_release_integration.md` を生成する。`make final` / `make release-patch` はstage 25からv1.3.9の32 MiB最終ROM、BPS、9-member決定論ZIP、`reports/generated/release_verification.md` を生成する。`make verify-release` はBPS完全往復とarchive禁止物を副作用なしで照合し、`make release-fresh-check` はtagged sourceの隔離worktreeから全chainを1回再構築してfinal/BPS/ZIPのbyte一致証跡を生成する。
- USER Species名: `make species-surface` が40 consumerと134個の6文字名を再監査し、`generated/engine/species/species_name_consumers.json` と `reports/generated/species_name_consumers.md` を生成する。exact-ROM smokeは戦闘メッセージとhealthbox OBJ tile、共有文字列経路、buffer canaryを検査する。
