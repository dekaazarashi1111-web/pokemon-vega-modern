...
- USER QOL release: `make qol-release-smoke` がstage 20→25のchainを確認し、同一stage 25でKanto/QOL-B、Factory、初戦、HM、戦闘規則/UI、わざメモリーを再観測して `build/stages/25_mgba_qol_release.json` と `reports/generated/qol_release_integration.md` を生成する。`make final` / `make release-patch` はstage 25からv1.3.6の32 MiB最終ROM、BPS、9-member決定論ZIP、`reports/generated/release_verification.md` を生成する。`make verify-release` はBPS完全往復とarchive禁止物を副作用なしで照合し、`make release-fresh-check` はtagged sourceの隔離worktreeから全chainを1回再構築してfinal/BPS/ZIPのbyte一致証跡を生成する。
