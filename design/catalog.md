...
- `tasks/task_graph.json`: T00〜T28の依存関係正本
...
- `design/codex_battle_architecture.md`: iPad RetroArch NCI、Codex CLI、6→3対戦、任意報酬の実装正本
- `tasks/T26_CODEX_BATTLE_BRIDGE.md`: Codex対戦mailboxと実iPad NCIのStage 43実証仕様
- `tasks/T27_CODEX_BATTLE_RUNTIME.md`: Codex操作6→3対戦のStage 44統合仕様
- `tasks/T28_CODEX_BATTLE_REWARDS_RELEASE.md`: 任意報酬・companion skill・iPad E2EのStage 45完成仕様
...
- `docs/wiki/stage61/README.md`: Stage61固定スナップショットのポケモン・技・特性・主要アイテム入手Wiki
...
- `scripts/build_stage61_wiki.py`: Stage61固定スナップショットと統合済み取得正本からMarkdown／JSONL Wikiを決定的生成・照合
...
- `scripts/build_qol_release.py`, `build/stages/25_mgba_qol_release.json`: stage 25上の初戦/HM/battle rules/UI/技管理と既存Kanto/Factoryを同一ROMで確認するv1.3.9統合契約
- `tools/release/`, `scripts/build_release.py`: T18/v1.3.9のBPS encode/decode、最終ROM、決定論release archive、安全監査、隔離fresh-checkout再構築
- `generated/engine/species/species_name_consumers.json`, `reports/generated/species_name_consumers.md`: 40個のstock Species名consumer、48個のstride/bound patch、134個の6文字名、戦闘nickname転送と表示上限のfail-closed監査
