...
`design/tasks_next.md` を正とする。T00〜T30と既存の明示USERタスクは完了している。`USER-20260824-STAGE51-WORLD-RUNTIME-E2E-REPAIR`はiPad配置済みStage55の実プレイ承認待ちで、唯一の正本IN_PROGRESSとして保持する。

- USER-20260824-STAGE51-WORLD-RUNTIME-E2E-REPAIR: IN_PROGRESS（`STAGE55_IPAD_PLACED_AWAITING_PLAY`）。正しい知恵の洞窟section `131`、`96/5 local 5`博士NPC、event-design TALK_OBJECT全15件の可視fallback共通修正はローカルPASS。Stage55 ROMと正常進行saveはiPadへ別basenameで配置・read-back済み。506 double／551番水道を含め、iPad通常プレイで再検証する。再開時は`docs/HANDOFF_STAGE53_WORLD_RUNTIME_REOPEN.md`だけを先に読む。
