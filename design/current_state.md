...
`design/tasks_next.md` を正とする。T00〜T30と既存の明示USERタスクは完了している。`USER-20260824-STAGE51-WORLD-RUNTIME-E2E-REPAIR`はStage53のローカル修復まで完了したが、iPad実プレイ承認待ちのため唯一の正本IN_PROGRESSとして保持する。

- USER-20260824-STAGE51-WORLD-RUNTIME-E2E-REPAIR: IN_PROGRESS（`PASS_LOCAL_IPAD_ROM_PLACED_AWAITING_PLAY`）。Stage53の全owner監査、18 fresh-core fixture×2 process、BPS、declared span外0、allocator overlap 0、iPad別名ROM配置はPASS。ユーザー実プレイ確認までDONEにしない。
