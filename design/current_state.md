...
- P01はDONE。P02〜P08は未完了で、release readyではない。P08の選択済み累積候補はStage77 `build/stages/77_modernization_p05_circus_suppression.gba`、SHA-256 `245133a4740dda9faa0663d321505ee793293d64b0b318d601fd91933b84973f`、CRC32 `F1CE0EAC`へ更新した。現行プレイ基準Stage62へは昇格していない。
- P02は6種のlevel＋所持道具分岐順を60 bytes修復し、実`GetEvolutionTargetSpecies`／`ItemEvolutionRemoval`で成立・不足・誤道具・通常分岐を独立2 process検証した。Stage71全受入のmGBAは2回とも製品判定前のハーネス導線で停止し、production runtimeは`UNJUDGED`、通常UI／scene後identity／fresh-core reloadはpendingとした。次回の累積run用ハーネスは既知正常save SHA-256 `f6bfdb107196ca22b012c1d12ee4bcdc8f5add309bbd3538447cd6e39c449bcb`をprocess別に私有コピーし、通常Continueでinput-ready fieldへ入った後だけparty／item fixtureを置く。P03はStage66/67で累積51,151経路、Stage73で5群の新規5,363経路を実consumerへ接続し、既存owner 35,207経路を別計上した。Stage74では残るmachine 26,279＋tutor 369の26,648経路を、殿堂入り後のわざメモリー内にfamily分離したarchiveとして直接供給した。Stage75ではOwn Tempo Rockruff 0744.01を内部条件フォームSpecies 1670へ接続し、38持越し経路の欠落ownerを0にした。新規runtime materializationは累積83,162、既存owner込みaccountedは118,369、直接供給残0のままで、P03の最終累積mGBAだけを残す。P02／P03とも全受入はまだ未完了。
...
- Stage70でMega用Species/Form 49件をID 1621〜1669として、28固定表とfront／back／palette／shiny／iconをROMへ接続した。既存Species 0〜1620の行はbyte一致を保持し、evolution表は1,670行・39 consumerへ再接続。Ability表は312→318行としてID 312〜317の差し替え可能な仮文言まで実装した。Stage71では49件のbase＋専用石の順方向行とMega→baseの逆方向行を追加し、既存Mega 80行を保持した。通常戦では既存project mechanic modeとMega Ring 580の両gate、Frontier／Linkでは既存のRing例外を継承する。交代時はMega維持、ひんし時はbaseへ戻る一方で使用済み状態を維持し、蘇生後の再Megaを拒否する。Browt／Pombon／GecquaはID・素材・runtimeを予約しない`NON_ADOPTED_USER_SCOPE`のまま。
- Stage72でAbility ID 312〜317（Dragonize／Eelevate／Fire Mane／Mega Sol／Piercing Drill／Spicy Spray）を6 Megaの全3 ability slotへu16のまま結合し、固定説明・rating・Mold Breaker表と29 battle hookを接続した。Ion DelugeよりDragonize、DragonizeよりElectrifyを優先する。Mega Solは使用者のUtility Umbrellaに妨げられず個人晴れの回復2/3を使い、実晴れで結果が同じ場合はpopupを重ねず、味方Flower Giftも誤発火させない。Eelevateはstate29の効果なし経路をstate30で狭くyieldさせ、通常KOとFuture Sight使用者がダブルの同陣営partner bankへ戻ったKOの双方で、後続の能力上昇script完了まで実使用者bankを保持する。未判明Ability Shieldを先読みしないvisual境界も接続した。focused 15件と再生成なしbyte比較はPASS、変更7,257 bytes／許可7,436 bytes／許可外0。
- Stage74 ROMは33,554,432 bytes、SHA-256 `481083bc50bd353955990375e3cc5e0a76f9b0f681ae54caa6c31f66ef22d65e`、CRC32 `C9929F79`。machineは40技×最大4ページ、tutorは1ページとし、固定TM／教え技slot、level／eggへ意味変換しない。Benjamin Butterfree退化時の合法技消去を防ぐため、全118,369選択経路から現行`BuildLearnableMoveset`＋直接archiveとの差分4,014経路／2,223 species-moveを削除防止専用表へ入れ、UI供給と経路勘定には加算しない。全1,621種の最大bufferは238/429、overflow 0。
- Stage75 ROMは33,554,432 bytes、SHA-256 `a179c024294f4f1bbf34eb603af255f6896265d9d8523344719b349f8a4495c3`、CRC32 `1511F429`。通常イワンコ1142を保存互換のまま残し、マイペース固定3slotの内部フォーム1670を追加した。通常1142から黄昏1263への17〜19時・Lv.25進化行を1670へ移し、野生1142の成功生成だけを追加RNGなしのpersonality固定1/8で1670へ変換する。繁殖は1263／1670だけを1670へ狭く解決し、他Speciesは親処理へexact delegateする。24表／310 pointer／19 count consumer、5 hook、allocation sequence 78と既存9 slice hash更新、38持越し経路owner欠落0を固定した。通常追加ポケモンの採用数は0のままで、1670は図鑑数を増やさない内部条件フォームである。
- Stage76 ROMは33,554,432 bytes、SHA-256 `f753f13720aeb5331cfc8a9bf9dd5fd4ad9ac34537356d20d76b73e0100100ac`、CRC32 `0A78B46A`。Mega Solのソーラービーム溜め時popup、Piercing DrillのAI上の予測Protect貫通1/4（最低1）、Spicy Sprayの意図的な味方発火評価を、pointer 1件＋hook 3件へ接続した。Detect正規化、Max Guard／ダイマックス、実Protectの二重1/4回避、direct／spread排他、Present／Future Sight／Doom Desire／Pollen Puff除外までfail closedで固定した。Eelevate専用交代AIはGround吸収とThousand Arrows／接地／Gravity／Mold Breaker／Ability Shieldを単純なAbility置換で保てないため、Stage75の候補2 siteをbyte不変のまま明示保留した。focused 17件、builder check、独立runtime／artifact監査はPASS、mGBAは最終累積1セットへ保留したためP05完了はまだ主張しない。
- Stage77 ROMは33,554,432 bytes、SHA-256 `245133a4740dda9faa0663d321505ee793293d64b0b318d601fd91933b84973f`、CRC32 `F1CE0EAC`。Battle Circus bit 26と特性無効bit 31が同時に立つ時だけ、Stage72の29 hook／33特性surfaceを各original trampolineへ委譲するdispatcherを追加した。通常戦はStage72 wrapperを維持し、Gastro Acid／Neutralizing Gas／Mold Breakerは既存のraw-zero抑制を変更しない。7本の12-byte hookは元r3をr12経由で復元し、残る22本はStage72互換のr3 scratchとした。親allocation 80行、Stage76のpointer 1＋hook 3、Eelevate保留2 siteをbyte不変で保持し、allowlist外変更0、BPS往復、focused 12件、独立監査をPASSした。mGBAとEelevate専用交代AIが残るためP05完了はまだ主張しない。
- Stage78 ROMは33,554,432 bytes、SHA-256 `98fde60231175492032f0e28ca16549a73ca5b29e3f37438b77c6e3c80e9d06b`、CRC32 `BFF0203C`。Eelevate専用交代AIを`FindMonThatAbsorbsOpponentsMove`のactive／party 2 siteへ接続し、有効な予測Ground攻撃に限ってAbility 313を比較用Earth Eater 298へ写像する。予測なし／switch／範囲外、status、Thousand Arrows、Gravity／Iron Ball／Ingrain／Smack Down、Gastro Acid、Battle Circus特性無効、Ability Shieldで保護されないMold Breaker系と残存Neutralizing Gasは保守的に拒否する。payloadは688 bytes、allocation sequence 81。32-case Python＋host C、focused 12件、builder check、BPS往復、allowlist外0、独立監査2件をPASSした。重いmGBAはStage79の最終累積1セットへ保留し、P05／release完了はまだ主張しない。P08 selected candidateは次の統合checkpointまでStage77のまま保持する。
- Stage79は製品ROMを変更しない最終累積mGBA基盤として`READY_NOT_RUN`。Stage78 commit `a98e6fea59db1f020bc902a1b1676699f8c06c41`、ROM／metadata／allocation／checkpoint、allocation 82行／sequence 81、P05 config／symbols／contractを既知hashへ固定した。P02、Mega Stone店、Floette、P03、49 Mega runtime、battle policy、P05の7領域を順次・再開可能に実行し、各領域は別のprivate ROM／save／state境界とする。Eelevateは32-case pure matrixに加え、active 13／party 13の実hook routeで非0 battler伝播、foe選択、0／FFFF／上限外fallback、接地・抑制・Neutralizing Gas・Ability Shield、continuation／register／SP ABIを観測する。focused 23件、dry-run 7/7 READY、strict compile 7/7、`check`をPASSし、mGBA processは0。重い実行後もscheduler/full P03/full P05/releaseは別条件として過大主張しない。
- Stage79の重い検証はGitHub Actionsの7-job matrixへ移管した。Stage78 ROM／metadata／allocation、P02 seed save、生成済みsymbol／auditを含む必須16ファイル（34,748,225 bytes）をprivate repositoryへ直接追跡し、clean checkoutだけで36件の固定identityを再構成できる。初回はlocal順次runのP02 `rare_candy_cancel_entry`でillegal opcode／field return timeoutとなり、後続6領域は未実行。GitHub側は`fail-fast: false`で全領域を独立実行し、PASS cacheを再利用して失敗領域だけを再実行できる。
...
- `USER-20260830-STAGE60-DISPLAY-NPC-PLACEMENT-AUDIT`はIN_PROGRESS。2026-09-03のVega既存trainer会話復元＋外来生態率調整にCodex runtime cold-boot入口復旧を重ねたcheckpointを最新とする。ChangeKitの手持ち・trainer ID・battle flagsは保持し、Vega既存1,030 encounterの原版会話を再接続した。固定追加文の855件、35-byte intro 478件、8-byte外付けintro 21件、固定post 855件を迂回し、勝利後と撃破済み再会話の継続先が異なる30件も原版どおり分離した。KANTO_NEW／ARCHIVE 272件は不変。通常外来生態48 entryは1種20%、2～3種30%、4～5種40%、6種以上50%とし、昼夜・大量発生・釣り・隠し47 entryはbyte不変。T501は追加8種で50%。同task単独の直近checkpointは旧Stage61 SHA-256 `60b83b8c50e54c3af42daa23b9d82e96c1b769d816ce28ef8bfda1d5005aff0e`で、現行プレイ基準には上記runtime hotfixを重ねた。strict全件監査は`DEFERRED_AUDIT`で、元taskはIN_PROGRESSを維持する。
- 上記を更新する最新checkpointはStage62 NPC配置整合性修復。明示追加282体だけを可変とし、既存2,830体をimmutable化した。Stage61が変更した既存316体／77 mapとruntime座標operand 2件を固定Vega／clean FireRed原典へ戻し、追加11体だけを安全な場所へ再配置した。全678 mapの最終配置、全5,333 script rootのobject操作、collision／到達port／通路component／warp・coord・BG／trainer sightを検査し、未認識参照と配置failureは0。`1/73 local 1`は`(4,11)`・elevation 3・movement 8・range `1/1`へ戻り、coord 9のexternal movement 5件＋remove 1件でposition-anchoredと判定した。mGBA 6 fixture×2 fresh processで、3穴・段差・ライバル戦・actor消去・南口・化石・通常save・fresh ContinueまでPASSした。strict全owner／全会話branch監査は`DEFERRED_AUDIT`として残り、元taskの`[>]`を維持する。

## 現在地

- USER-20260830-STAGE60-DISPLAY-NPC-PLACEMENT-AUDITのNPC配置修復checkpointとしてStage62を生成した。入力Stage61 SHA-256 `734541807df91ca6f82211b57e0a56e6af6c1c70ec46b9f89cd6a89b3f701f3b`は不変。出力SHA-256は`d97a0d4a6cd6f8f77a1503a5ac6d473b0e94c4892e3d5a94098497ce35cb6e6f`、変更879 byte、declared外0、script pointer変更0、trainer type／sight変更0、Stage61差分BPSとclean直接BPSは完全往復した。正本レポートは`reports/generated/stage62_npc_placement_integrity_audit.json`、`reports/generated/stage62_npc_catalog.json`、`reports/generated/stage62_wisdom_cave_regression.json`、`reports/generated/stage62_wisdom_cave_mgba.json`。
- USER-20260903-STAGE61-RUNTIME-HOTFIXで、`gTMHMMoves`をTM120＋HM8、`gTutorMoves`をV4教え技64へ再配置した。TM51–58の互換bitはV4入力から再構築し、旧HM互換はindex 121–128へ移送した。固定CFRU-JPの16 consumer hook、canonical／alias／expanded TM・HM item境界、全64教え技Move IDとslot 64互換境界をexact ROMで検証した。Item 347／348のtypeを`BAG_MENU(4)`から`FIELD(2)`へ直した。続くUSER-20260903-STAGE61-MOVE-MEMORY-STALE-BATTLE-FLAG-FIXでは、通常fieldでも直前のtrainer battle bit `0x4`が残る実機状態を特定し、field-use item入口で保証済みの戦闘中判定を重複していたcontext gateから`gBattleTypeFlags`だけを除外した。facility／Raid拒否は維持し、実Bag経路で両専用menu、Bキャンセル、おもいだす→party cancel、夜固定→mode 2、field復帰、およびstale battle flag状態のわざメモリー起動を独立2 process一致でPASSした。ROMはSHA-256 `734541807df91ca6f82211b57e0a56e6af6c1c70ec46b9f89cd6a89b3f701f3b`、CRC32 `232D05EA`。iPadへROMだけを原子的に配置し、read-back byte一致、save 57件不変を確認した。versioned CLI 2.5.1のprotocolも同じROM identityへ再固定した。WikiはTM/HM 128枠・教え技64枠の現ROM抽出へ再生成した。
