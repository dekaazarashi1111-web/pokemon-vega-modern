...
- ユーザーが現在遊ぶ基準は`design/active_play_baseline.md`に固定した。対象は`build/stages/61_critical_release_candidate.gba`、33,554,432 bytes、SHA-256 `44e951e20e7985b6f4480a04ff997cc77e6305dd0945eb90cfed7e3c6d85ae4e`、CRC32 `21A93A0E`。V4のTM120＋HM8と教え技64を実行時table／consumerへ接続し、TM51–58と旧HMの互換slot衝突を解消した。わざメモリー／せいたいレーダーはfield item復帰経路へ修正し、Bagからの起動、選択、キャンセル、field復帰をmGBA独立2 processでPASSした。iPadへ同一SHAのROMだけを原子的に配置し、read-back byte一致、通常プレイsave 57件不変を確認した。Codex対戦、任意報酬、Windowsカタログ送付、Box 14移動、バグ再現・修正は明示的な切替までこのROMを使う。
...
- USER-20260903-STAGE61-RUNTIME-HOTFIXで、`gTMHMMoves`をTM120＋HM8、`gTutorMoves`をV4教え技64へ再配置した。TM51–58の互換bitはV4入力から再構築し、旧HM互換はindex 121–128へ移送した。固定CFRU-JPの16 consumer hook、canonical／alias／expanded TM・HM item境界、全64教え技Move IDとslot 64互換境界をexact ROMで検証した。Item 347／348のtypeを`BAG_MENU(4)`から`FIELD(2)`へ直し、実Bag経路で両専用menu、Bキャンセル、おもいだす→party cancel、夜固定→mode 2、field復帰を独立2 process一致でPASSした。ROMはSHA-256 `44e951e20e7985b6f4480a04ff997cc77e6305dd0945eb90cfed7e3c6d85ae4e`、CRC32 `21A93A0E`。iPadへROMだけを原子的に配置し、read-back byte一致、save 57件不変を確認した。versioned CLI 2.5.1のprotocolも同じROM identityへ再固定した。WikiはTM/HM 128枠・教え技64枠の現ROM抽出へ再生成した。
