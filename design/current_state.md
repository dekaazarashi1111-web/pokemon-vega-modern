...
`design/tasks_next.md` を正とする。T00〜T30と既存の明示USERタスクは完了している。`USER-20260824-STAGE51-WORLD-RUNTIME-E2E-REPAIR`はローカル修復とStage52のiPad別名配置まで完了したが、実プレイ承認待ちのため唯一の正本IN_PROGRESSとして保持する。

- USER-20260824-STAGE51-WORLD-RUNTIME-E2E-REPAIR: IN_PROGRESS（`PASS_LOCAL_AWAITING_IPAD`）。Stage52の全owner監査、10 fresh-core fixture×2 process、BPS、iPad別名配置はPASS。ユーザー実プレイ確認までDONEにしない。
