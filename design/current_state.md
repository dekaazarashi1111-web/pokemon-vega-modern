...
- P01はDONE。P02〜P08は未完了で、release readyではない。P08の選択済み累積候補はStage69のままだが、開発中の最新累積checkpointはStage73 `build/stages/73_modernization_p03_consumer_runtime.gba`、SHA-256 `25329a1d5dd71a4f3c0adff8b337af1c4b3496e0aae64439ed2adebe338ce26a`、CRC32 `B4907165`。現行プレイ基準Stage62へは昇格していない。
- P02は6種のlevel＋所持道具分岐順を60 bytes修復し、実`GetEvolutionTargetSpecies`／`ItemEvolutionRemoval`で成立・不足・誤道具・通常分岐を独立2 process検証した。Stage71全受入のmGBAは2回とも製品判定前のハーネス導線で停止し、production runtimeは`UNJUDGED`、通常UI／scene後identity／fresh-core reloadはpendingとした。次回の累積run用ハーネスは既知正常save SHA-256 `f6bfdb107196ca22b012c1d12ee4bcdc8f5add309bbd3538447cd6e39c449bcb`をprocess別に私有コピーし、通常Continueでinput-ready fieldへ入った後だけparty／item fixtureを置く。P03はStage66/67で累積51,151経路を実consumerへ接続済み。Stage73では保留5群40,570経路について、条件付きegg 40、shared egg 5,023、reminder 295、ロトムform move 5の計5,363を新規runtimeへ実装し、既存Light Ball owner 1、pre-evolutionの4技保持35,141、既存／generic form owner 65の計35,207を既存ownerとして照合した。残る直接供給はmachine 26,279＋tutor 369の26,648経路で、P02／P03とも全受入は未完了。
