...
`design/tasks_next.md` を正とする。T00〜T30と既存の明示USERタスクは完了している。`USER-20260824-STAGE51-WORLD-RUNTIME-E2E-REPAIR`はStage53のfixture誤同定と博士風NPC未特定により再調査中で、唯一の正本IN_PROGRESSとして保持する。

- USER-20260824-STAGE51-WORLD-RUNTIME-E2E-REPAIR: IN_PROGRESS（`STAGE53_REJECTED_WRONG_FIXTURE_AND_PROFESSOR_UNRESOLVED`）。正しい知恵の洞窟section `131`と博士風NPCを通常プレイ導線から再同定し、506 double／551番水道を含む共通runtimeを再検証する。再開時は`docs/HANDOFF_STAGE53_WORLD_RUNTIME_REOPEN.md`だけを先に読む。
