...
- P01はDONE。P02〜P08は未完了で、release readyではない。P08の選択済み累積候補はStage69のままだが、開発中の最新累積checkpointはStage72 `build/stages/72_modernization_p05_ability_rom_runtime.gba`、SHA-256 `f27411a2dcef2ec2c1f3c06de624b24838683f5e77017fafa9bf445edc00d059`、CRC32 `F981D1CB`。現行プレイ基準Stage62へは昇格していない。
- P02は6種のlevel＋所持道具分岐順を60 bytes修復し、実`GetEvolutionTargetSpecies`／`ItemEvolutionRemoval`で成立・不足・誤道具・通常分岐を独立2 process検証した。Stage71全受入のmGBAは2回とも製品判定前のハーネス導線で停止し、production runtimeは`UNJUDGED`、通常UI／scene後identity／fresh-core reloadはpendingとした。次回の累積run用ハーネスは既知正常save SHA-256 `f6bfdb107196ca22b012c1d12ee4bcdc8f5add309bbd3538447cd6e39c449bcb`をprocess別に私有コピーし、通常Continueでinput-ready fieldへ入った後だけparty／item fixtureを置く。P03はStage66の47,548経路へ進化時341、既存教え技740、通常タマゴ2,522の計3,603経路を追加し、累積51,151経路を実consumerへ接続した。選択済み残経路は67,218。Stage73 preflightではこのうち条件付きegg 41、shared egg 5,023、pre-evolution carry 35,141、reminder 295、form change 70の計40,570を全118,528 source routeから再現可能に分離したが、新規ROM materializationは0で、P02／P03とも全受入は未完了。
