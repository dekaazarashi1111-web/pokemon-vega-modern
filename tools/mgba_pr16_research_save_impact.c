...
#define SI_ROM "23d584095f7bc0691e1582da447d6cd9a389d8698e061ea2de9f6eac03b63f48"
...
int main(int argc,char**argv){
 ...
 si_need(kind!=0,"operation");unsigned phase=qol_number(argv[4],"phase");si_need(phase<=3,"phase 0 success, 1/2 fault, 3 powercut");
 ...
 si_need(result==(phase==0?0:phase==3?UINT32_MAX:13),"transaction result");
 si_need(saves==(phase==0?2:phase==1?0:1)&&!loads&&pa==1&&pb==(phase==1?0:1),"real save delegate/phase counts");
 si_need(read32(c,SI_COUNTER)==base+saves,"real native save counter");si_event(c,"returned_or_cut",item);
 if(phase==1){uint8_t now[64];si_owner(c,now);si_need(!memcmp(before,now,64)&&si_item(c,item)==qty,"phase1 rollback");}
 ...
 si_need(si_read32(c,SI_OWNER+36)==(phase==1?1:2)&&si_read32(c,SI_OWNER+40)==(apply&&kind==1?0x52450001U:0),"durable transaction identity");
