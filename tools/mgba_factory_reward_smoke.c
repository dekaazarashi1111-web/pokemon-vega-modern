/* Focused stage28 exact-ROM smoke for Factory Trial manifest rewards. */
...
#define REWARD_MAX_CALL_STEPS UINT64_C(30000000)
#define REWARD_SAVE_FILE_SIZE UINT32_C(0x20000)
#define REWARD_ABI_MARKER UINT32_C(0xB928)
...
int main(int argc, char **argv)
{
    ...
    uint32_t old_complete = parse_u32(argv[8], "old completion");
    const uint32_t entrypoints[] = {probe, complete, finalize, ensure, old_complete};
    for (size_t index = 0U;
    ...
    if (!rom_pointer(complete_script) || complete == old_complete)
    ...
    if (read8(core, complete_script) != 0x23U
        ...
        reward_die("completion callnative wrapper is not physically bound");
    ...
    if (reward_call_thumb(core, probe, 0U, 0U, 0U, 0U)
        ...
        reward_die("Factory reward ABI probe failed");
    ...
    if (reward_invoke(core, complete) != REWARD_RESULT)
        reward_die("first completion wrapper returned the wrong result");
    uint32_t factory = REWARD_LEDGER + REWARD_FACTORY_OFFSET;
    if (read16(core, factory + REWARD_BP_OFFSET) != 112U
        || read16(core, factory + REWARD_CURRENT_STREAK_OFFSET) != 3U
        || read16(core, factory + REWARD_BEST_STREAK_OFFSET) != 3U
        ...
        reward_die("first-clear item/BP/claim state differs");
    ...
    if (reward_invoke(core, ensure) != 1U
        ...
        || read32(core, factory + REWARD_CLAIM_BITS_OFFSET) != 0x1FU)
        reward_die("sector31 did not reload reward ledger");
    reward_require_flash_ledger(core, "first-clear reload");
    reward_require_party(core, original, original_count, "first-clear reload");

    reward_phase("streak-catchup");
    const uint16_t first_credits[4] = {1U, 0U, 0U, 0U};
    reward_prepare_completion(core, finalize, original, original_count,
                              112U, 21U, 0x1FU, first_credits);
    if (reward_invoke(core, complete) != REWARD_RESULT
        || read16(core, factory + REWARD_BP_OFFSET) != 121U
        || read32(core, factory + REWARD_CLAIM_BITS_OFFSET) != 0xFFU
        || reward_item_count(core, REWARD_ITEM_XS) != 5U
        || reward_item_count(core, REWARD_ITEM_S) != 2U)
        reward_die("streak catch-up or duplicate suppression differs");
    reward_expect_credits(core, 1U, 1U, 1U, 1U, "streak catch-up");
    reward_require_party(core, original, original_count, "streak catch-up");
    reward_require_flash_ledger(core, "streak catch-up");

    reward_phase("repeat-clear");
    const uint16_t all_credits[4] = {1U, 1U, 1U, 1U};
    reward_prepare_completion(core, finalize, original, original_count,
                              121U, 21U, 0xFFU, all_credits);
    if (reward_invoke(core, complete) != REWARD_RESULT
        || read16(core, factory + REWARD_BP_OFFSET) != 130U
        || read32(core, factory + REWARD_CLAIM_BITS_OFFSET) != 0xFFU
        || reward_item_count(core, REWARD_ITEM_XS) != 5U
        || reward_item_count(core, REWARD_ITEM_S) != 2U)
        reward_die("repeat clear duplicated manifest rewards");
    reward_expect_credits(core, 1U, 1U, 1U, 1U, "repeat clear");
    reward_require_party(core, original, original_count, "repeat clear");
    reward_require_flash_ledger(core, "repeat clear");

    reward_phase("bag-full");
    reward_empty_item(core, REWARD_ITEM_XS);
    reward_empty_item(core, REWARD_ITEM_S);
    uint32_t full_count = reward_fill_item(core, REWARD_ITEM_XS);
    reward_prepare_completion(core, finalize, original, original_count,
                              200U, 3U, 1U, zero_credits);
    if (reward_invoke(core, complete) != REWARD_RESULT
        || read16(core, factory + REWARD_BP_OFFSET) != 209U
        || read32(core, factory + REWARD_CLAIM_BITS_OFFSET) != 1U
        || reward_item_count(core, REWARD_ITEM_XS) != full_count
        || reward_item_count(core, REWARD_ITEM_S) != 0U)
        reward_die("bag-full bonus was not deferred after base completion");
    reward_expect_credits(core, 0U, 0U, 0U, 0U, "bag-full");
    reward_require_party(core, original, original_count, "bag-full");
    reward_require_flash_ledger(core, "bag-full");

    printf("{\"schema_version\":1,\"status\":\"PASS\","
           "\"checks\":{\"completion_wrapper_binding\":true,"
           "\"probe_and_save_init\":true,\"first_clear_bonus\":true,"
           "\"streak_credit_catchup\":true,"
           "\"repeat_no_duplication\":true,"
           "\"bag_full_deferred_bonus\":true,"
           "\"normal_save_item_reload\":true,"
           "\"sector31_ledger_match\":true,"
           "\"exact_party_restore\":true},"
           "\"first_clear_bp\":112,\"repeat_bp\":130,"
