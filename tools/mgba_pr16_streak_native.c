...
static void sc_event(struct mCore *c,const char *label,unsigned battle){
    bp_require(c,sc_events++<64U,"Circus streak event budget exceeded");
    fprintf(stderr,"CIRCUS_STREAK {\"label\":\"%s\",\"frame\":%u,\"battle\":%u,\"callback2\":%u,\"script\":%u,\"newbs\":%u,\"outcome\":%u,\"flags\":%u,\"types\":%u,\"count\":%u,\"bp\":%u,\"save_counter\":%u,\"pending\":%u,\"snapshot\":%u,\"marker\":%u,\"order\":[",
...
static void sc_select(struct mCore *c){
    ...
    sc_event(c,"selected",0U);b_press(c,QOL_KEY_A,120U);
}
static void sc_launch_battle(struct mCore *c,unsigned battle){
    ...
    bp_require(c,sc_phase(c)==2U && sc_current(c)==battle && sc_best(c)==battle,"Circus prepare did not arm dedicated streak");
    ...
    sc_event(c,"confirmation",battle);sc_frozen(c);
    for(unsigned f=0;f<18000U;++f){
    ...
    bp_require(c,sc_phase(c)==2U && sc_current(c)==battle && sc_best(c)==battle,"Circus draw changed dedicated streak");
    sc_frozen(c);sc_event(c,"action",battle);++sc_battles;
}
...
static unsigned sc_finish_battle(struct mCore *c,unsigned battle){
    ...
    while(b_frames-w.start<90000U){
        ...
        if(value && !outcome){outcome=value;bp_require(c,value==1U || value==2U,"Circus outcome not native win/loss");sc_event(c,"outcome",battle);}
        ...
        if(outcome && !read32(c,ADDR_NEW_BATTLE_STRUCT_POINTER) && sc_phase(c)!=2U){
            if((outcome==1U && sc_current(c)==battle+1U && sc_best(c)==battle+1U)
                || (outcome==2U && sc_phase(c)==0U && !sc_current(c) && sc_best(c)==battle)){
                b_frames_run(c,0,30U);sc_event(c,"settled",battle);sc_turns+=w.turns;sc_switches+=w.switches;
    ...
    sc_event(c,"timeout",battle);bp_require(c,false,"Circus native settle timeout");return 0U;
}
static void sc_returned(struct mCore *c){
    ...
    bp_require(c,sc_best(c)==sc_wins && sc_current(c)==(sc_losses?0U:3U)
        && read16(c,BP_F(battle_points))==(sc_losses?0U:9U),"Circus final streak/BP differs");sc_frozen(c);
}
int main(int argc,char **argv){
    if(argc==3 && !strcmp(argv[1],"--guard-check"))a_guard_check(argv[2]);
    if(argc!=7 || strcmp(argv[5],"circus-streak-batch-save"))return 2;
    ...
    printf("{\"schema_version\":1,\"status\":\"PASS_CIRCUS_STREAK_NATIVE\",\"case\":\"%s\",\"candidate_sha256\":\"%s\",\"wins\":%u,\"losses\":%u,\"battles\":%u,\"turns\":%u,\"switches\":%u,\"forced_identity_checks\":%u,",argv[5],hash,sc_wins,sc_losses,sc_battles,sc_turns,sc_switches,sc_id_checks);
