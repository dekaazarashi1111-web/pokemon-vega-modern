    ...
    for(unsigned i=0;i<29U;++i){
        ...
        if(call->active){
            ...
            if(pc==route->normal || pc==route->suppressed){
                ...
                fprintf(stderr,"P08_ORDINARY_CALL {\"kind\":\"dispatch\",\"name\":\"%s\",\"frame\":%u,\"flags\":%u,\"types\":%u,\"root\":%u,\"delegate\":%u,\"expected_normal\":%u,\"preserve_r3\":%s,\"r3_before\":%u,\"r3_after\":%u,\"host_writes\":0,\"host_calls\":0,",route->name,b_frames,flags,types,route->root,pc,route->normal,route->preserve_r3?"true":"false",call->r3,after);
                po_pcs(call);fprintf(stderr,"}\n");call->active=false;++po_delegates;
                bp_require(c,pc==route->normal && (!route->preserve_r3 || call->r3==after),"natural Stage77 delegate or r3 differs");
            }
        }
