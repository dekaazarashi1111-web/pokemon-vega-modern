...
static uint32_t hm_field_capability;
static uint32_t hm_field_callback_table;
...
static struct HMCase HM_CASES[] = {
    {"HM01_CUT", 15, 339, 1, 0},
    {"HM02_FLY", 19, 340, 2, 0},
    {"HM03_SURF", 57, 341, 4, 0},
    {"HM04_STRENGTH", 70, 342, 3, 0},
    {"HM05_FLASH", 148, 343, 0, 0},
    {"HM06_ROCK_SMASH", 249, 344, 5, 0},
    {"HM07_WATERFALL", 127, 345, 6, 0},
    {"HM08_DIVE", 291, 346, 14, 0},
};
...
int main(int argc, char **argv)
{
    if (argc != 13) {
        fprintf(stderr,
                "usage: %s ROM EXPECTED_ROM_SHA256 FIELD_CAPABILITY "
                "CALLBACK_TABLE ORIGINAL_CALLBACK_X8\n",
                argv[0]);
        return 2;
    }
    char rom_sha256[65];
    ...
    hm_field_capability = parse_address(argv[3]);
    hm_field_callback_table = parse_address(argv[4]);
    for (unsigned index = 0; index < ARRAY_LEN(HM_CASES); ++index)
        HM_CASES[index].original_callback = parse_address(argv[5 + index]);
