/* Issue19: 最初の質問でB拒否。受入済み4境界のmainは呼ばない。
...
int main(int argc,char **argv){
    ...
    for(unsigned f=0;f<48000;++f){
        ...
        a_require(cb!=P03F_SUMMARY_CB,"first-question refusal entered forbidden summary");
        unsigned script=read32(c,X_SCRIPT_SLOT);
        unsigned opcode=(script>=0x08000000U && script<0x0A000000U)?read8(c,script):0xFFU;
        unsigned key=0;
        if(f%30==0){
            if(opcode==0x5AU || opcode==0x5BU){
                a_require(level_frame && read16(c,0x02023F82U)==497U,"first refusal owner/pending move");
                key=opcode==0x5AU?QOL_KEY_B:QOL_KEY_A;
                if(opcode==0x5AU){
                    a_require(!stop_frame && ++refusal_pulses<=8,"first question reentered or pulse bound");
                    if(!refusal_frame)refusal_frame=lb_frames;
                }else{
                    a_require(refusal_frame && ++stop_pulses<=8,"stop confirmation before refusal");
                    if(!stop_frame)stop_frame=lb_frames;
                }
                fprintf(stderr,"FIRST_REFUSAL frame=%u script=%08x opcode=%u key=%u pending=%u\n",lb_frames,script,opcode,key,read16(c,0x02023F82U));
            }else key=QOL_KEY_A;
        }
        lb_frame(c,key);
    ...
    a_require(prompts==1 && refusal_pulses && stop_pulses && level_frame<refusal_frame && refusal_frame<stop_frame && stop_frame<returned && !summaries && !selections,"first refusal and confirmation without summary");n_slots(c,after,afterpp,level);
    a_require(read16(c,QOL_PLAYER_PARTY+86)>0 && read16(c,QOL_PLAYER_PARTY+86)<=read16(c,QOL_PLAYER_PARTY+88),"native EXP health bounds");
    ...
    printf(",\"first_refusal_pulses\":%u,\"stop_confirmation_pulses\":%u,\"first_refusal_frame\":%u,\"stop_confirmation_frame\":%u,\"summary_never_opened\":true,\"refusal_opcode\":90,\"stop_opcode\":91",refusal_pulses,stop_pulses,refusal_frame,stop_frame);
