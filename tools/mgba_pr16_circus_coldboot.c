/* Cold boot bridgeの影響検査。既存Factoryの自動復旧Save2回と通常Save1回を区別。 */
...
int main(int argc,char **argv)
{
    ...
    bp_require(c,memcmp(armed,recovered,sizeof(armed)) && read32(c,P03_SAVE_COUNTER)==sc_counter+2U
    ...
    bp_require(c,!memcmp(recovered,loaded,sizeof(loaded)) && !memcmp(inventory,inventory_after,sizeof(inventory))
        && read32(c,P03_SAVE_COUNTER)==sc_counter+3U,"Circus recovery repeated abort or lost64/inventory/counter");
    ...
    printf("\"save_counter_before\":%u,\"save_counter_after\":%u,\"manual_saves\":1,\"automatic_recovery_saves\":2,\"fresh_cores\":3,\"owner_bytes_verified\":64,\"party_bytes_verified\":600,\"host_write_barriers\":7,\"total_frames\":%u,\"events\":%u,\"input_only_after_guard\":true,\"physical_admission_accepted\":false,\"suppression_accepted\":false,\"release_ready\":false,\"warnings_errors\":0}\n",sc_counter,final_counter,b_frames,sc_events);
    return 0;
}
