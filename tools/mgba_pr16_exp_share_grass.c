...
int main(int argc,char **argv){
    ...
    create_mon(c,QOL_PLAYER_PARTY,e_sid,v->level);write8(c,QOL_PLAYER_PARTY_COUNT,1);
    ...
    lb_path(c,5,lb_town_path,sizeof(lb_town_path)/sizeof(*lb_town_path),false);lb_step(c,QOL_KEY_UP);lb_position(c,96,17,11,39);
    /* 草むら最後の手前まで1体で通常移動。2体でのevent横断は今回のscope外。 */
    ...
    unsigned char before[200]={0},party[200]={0},again[200]={0};e_party(c,"fixture",before);lb_position(c,96,17,13,30);
    ...
    lb_path(c,17,lb_grass_path+approach-1,1,true);
