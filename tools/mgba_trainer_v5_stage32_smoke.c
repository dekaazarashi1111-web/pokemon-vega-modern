...
static const uint32_t TV32_HOOK_SITES[] = {
    ...
    UINT32_C(0x0810D93C), UINT32_C(0x090DD198),
    UINT32_C(0x0807F948),
};
...
static void tv32_die(const char *message) {
    fprintf(stderr, "mgba-trainer-v5-stage32: %s\n", message);
    exit(1);
}
...
static void tv32_trace(const char *phase) {
    ...
    fprintf(stderr, "mgba-trainer-v5-stage32 phase: %s\n", phase);
    fflush(stderr);
}
...
static uint32_t tv32_parse(const char *text, const char *label) {
    ...
    if (errno || !end || *end || value > UINT32_MAX) {
        fprintf(stderr, "mgba-trainer-v5-stage32: invalid %s\n", label);
        exit(2);
    }
...
static bool tv34_exact_rebinds(
    ...
    static const struct ExactCase cases[] = {
        ...
        {UINT32_C(0x08E03481), 0U, 119U, 1044U},
    ...
    for (unsigned index = 0; index < ARRAY_LEN(cases); ++index) {
        const struct ExactCase *test = &cases[index];
        restore_snapshot(core, field);
        uint8_t observed_kind = read8(core, test->data_address);
        /* kind-3 commands are not halfword aligned.  Byte assembly observes
         * the ROM bytes exactly instead of GBA's rotated unaligned LDRH. */
        ...
        if (!one) {
            fprintf(stderr,
                    "exact-rebind[%u] actual: data=0x%08" PRIx32
...
static bool tv33_batch02_sidecar(
    struct mCore *core, const struct Snapshot *field
) {
    (void)tv32_setup_trainer(core, field, 1201U, 1U, 0U);
    uint32_t first = ADDR_ENEMY_PARTY;
    uint32_t second = ADDR_ENEMY_PARTY + POKEMON_SIZE;
    uint32_t first_species =
        call_preserving(core, BATTLE_CORE_GET_MON_DATA, first, 11U, 0U, 0U);
    uint32_t second_species =
        call_preserving(core, BATTLE_CORE_GET_MON_DATA, second, 11U, 0U, 0U);
    uint8_t first_nature = read8(core, first + TV32_MON_NATURE_MINT);
    uint8_t second_nature = read8(core, second + TV32_MON_NATURE_MINT);
    uint32_t first_iv = read32(core, first + TV32_MON_IV_BITS);
    uint32_t second_iv = read32(core, second + TV32_MON_IV_BITS);
    bool ev_ok = true;
    for (unsigned ev = 0; ev < 6U; ++ev) {
        if (read8(core, first + TV32_MON_EV_HP + ev) != 0U
            || read8(core, second + TV32_MON_EV_HP + ev) != 0U)
            ev_ok = false;
    }
    uint16_t first_active_ability =
        read16(core, ADDR_BATTLE_MONS + BATTLE_MON_SIZE + 0x38U);
    bool passed = first_species == 4U && second_species == 1294U
        && first_nature == 1U && second_nature == 14U
        && tv32_repeated_iv(first_iv, 8U)
        && tv32_repeated_iv(second_iv, 10U)
        && ev_ok && first_active_ability == 66U;
    if (!passed) {
        fprintf(stderr,
                "batch02-sidecar actual: species=%" PRIu32 "/%" PRIu32
...
int main(int argc, char **argv) {
    if (argc != 10) {
        fprintf(stderr,
                ...
                "TRAINER_HAS REMATCH CONFIGURE TABLE\n", argv[0]);
        return 2;
    }
    ...
    uint32_t configure = tv32_parse(argv[8], "configure trainer battle");
    uint32_t table = tv32_parse(argv[9], "trainer table");
