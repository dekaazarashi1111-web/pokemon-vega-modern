...
このワークスペースでは、`design/tasks_next.md` の依存READYタスクから効果の高い1件を選び、検証と証跡を残しながら変更を進める。上からの並びは既定優先度であり、待ち時間削減やcritical path短縮のために別のREADYタスクを選んでよい。
...
- 再現性を壊さない。
- 証跡を残す。
- 品質と再現性を完了ゲートで担保し、待ち時間と重複作業を最小化する。

## 0.1 品質方針

- 完了時の品質と完成度を優先しつつ、反復中は最短の検証と動く縦切りで高速に進める。
...
- Git管理内と再生成領域の可逆操作は確認を待たずに進める。大きな変更前は必要に応じて小さなcheckpoint commitを作る。

## 1. ソースオブトゥルース

- タスク状態と既定優先順: `design/tasks_next.md`
...
1. `[>]` があれば再開し、なければ `taskctl.py next` が示す依存READY候補から、既定では `PRIMARY`、待ち時間やfan-outに利点があれば別候補を1件選んで `[>]` にする。
2. 同時に複数のタスク状態を変更しない。
3. タスク目的に必要な調査・設計・実装・検証は広げてよいが、目的に無関係な改善はしない。
4. 検証は可能な限り実行する。
5. PASSしたらログを追記し、`taskctl.py done` で `[>] -> [x]` と互換ミラーを同期する。意図した差分をstageし、軽量なtask graph検査と最終indexのprivate guard後に、状態・ログを含めて必ずコミットする。
...
- `PRIMARY`: キューの既定優先順にある推奨タスク。強制順ではない。
- `PARALLEL_PREP`: 依存上は同じくREADYの候補。正本IN_PROGRESSがない時は効率上の理由で選択してよく、実行中タスクがある時は所有ファイルを分けた先行作業に使う。

親側の正本タスクは同時に1件だけ `[>]` とする。`make plan` のwaveは並列準備可能な集合であり、同時に複数を `[>]` にする指示ではない。サブエージェントや別レーンの成果は、対象タスクを正本へ統合する時点で親が差分と対象テストを確認し、タスク単位のログとコミットを作る。
...
- 既存のユーザー変更を勝手に戻さない。
- 長い実装のcheckpointにはWIP commitを使ってよい。タスク完了commitは受入条件PASS後に別途作る。
