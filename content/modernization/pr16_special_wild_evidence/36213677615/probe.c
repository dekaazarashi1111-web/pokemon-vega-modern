...
static uint32_t observed(struct mCore*c,uint32_t function,uint32_t r0,uint32_t r1,unsigned attempt,uint32_t qb,uint32_t qe,unsigned*special,unsigned*apply){
    ...
    uint64_t steps=0;sw_post_calls=0;sw_special_move=0;guard(c);*special=0;*apply=0;
    for(;;){
        ...
        if(at==0x09114698U && lr>=qb && lr<qe && (uint32_t)read_register(c,"r0")==ENEMY_PARTY && read_register(c,"r2")==3){
            sw_special_move=(uint16_t)read_register(c,"r1");++*special;printf("{\"event\":\"special_setter\",\"attempt\":%u,\"step\":%" PRIu64 ",\"pc\":%u,\"lr\":%u,\"move\":%u,\"slot\":3}\n",attempt,steps,at,lr,(unsigned)read_register(c,"r1"));
        }
        if(at==0x09392722U||at==0x0939274AU){++sw_post_calls;dump_mon(c,"before_post",attempt,steps);}
...
int saved_probe_main(int argc,char**argv){
