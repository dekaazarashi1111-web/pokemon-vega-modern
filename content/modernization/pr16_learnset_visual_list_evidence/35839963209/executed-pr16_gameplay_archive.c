...
static bool v_list_shot,v_summary_shot;
static unsigned v_list_frame,v_summary_frame,v_summary_ready,v_list_ready;
...
static struct ATrace a_scene(struct mCore*c,unsigned family,unsigned page,unsigned index,int slot,unsigned action,unsigned count,const unsigned moves[40]) {
    ...
    for(unsigned f=0;f<A_MAX_FRAMES;f++) {
        ...
        /* State 4 is an interactive list on this candidate, not state 6 only.
         * Read-only capture waits for rendering; no palette or VRAM writes. */
        if((state==4 || state==6) && t.list && read8(c,p+0x9eb)==index) {
            if(++v_list_ready==90U) {
        ...
        if(f%30==0) {
            if(menu==6) {
                ...
                if(p03f_task(c,P03F_SUMMARY_TASK) && read8(c,q+P03F_SUMMARY_STATE)==2 && !summary_selected && v_summary_shot) {
...
int gameplay_old_archive(int argc,char**argv) {
