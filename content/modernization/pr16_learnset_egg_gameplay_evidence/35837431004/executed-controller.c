...
#define B_SCOPE "ISSUE19_CONDITIONAL_EGG_DAYCARE_HATCH_SAVE_CONTINUE"
...
#define B_ROM_SHA "6e88a021785bfa7cf00e26d7f2433c380602d830e94e1d2fc31e3198cda31df2"
#define B_SEED_SHA "f6bfdb107196ca22b012c1d12ee4bcdc8f5add309bbd3538447cd6e39c449bcb"

struct BCase {
    const char *name;
    unsigned father[4], mother[4], father_item, mother_item, child[4], mother_species, child_species, cycles;
};
static const struct BCase b_cases[] = {
    {"lightball-father", {175,440,33,0}, {273,45,0,0},202,0,{84,175,273,344},25,24,10},
    {"lightball-mother", {175,33,0,0}, {273,440,0,0},0,202,{84,175,273,344},25,24,10},
    {"no-lightball-removed", {344,440,33,0}, {344,440,45,0},0,0,{39,84,0,0},25,24,10},
    {"lightball-only-removed", {440,33,0,0}, {45,440,0,0},202,0,{39,84,344,0},25,24,10},
    {"duplicate-full-control", {175,273,0,0}, {175,273,0,0},0,0,{39,84,175,273},25,24,10},
    {"mother-incense-eligible", {84,39,0,0}, {69,461,464,357},0,862,{1,539,69,0},365,364,40},
    {"father-incense-removed", {84,39,0,0}, {461,464,357,0},862,0,{1,539,0,0},365,364,40},
    {"no-incense-control", {84,39,0,0}, {461,464,357,0},0,0,{204,343,539,549},365,365,40},
};
struct BTrace {
...
static void b_create(struct mCore *c,unsigned dst,unsigned species,unsigned pid,unsigned ot) {
    ...
    (void)call_rom_args(c,BATTLE_CORE_CREATE_MON,dst,species,20U,31U);
    bool ok=restore_host_call_stack(c,&stack);restore_cpu_state(c,&original);
    a_require(ok && b_data(c,dst,11U)==species && b_data(c,dst,0U)==pid
...
static void b_child(struct mCore *c,const struct BCase *v,unsigned egg,unsigned pp[4]) {
    ...
    a_require(b_data(c,B_CHILD,11U)==v->child_species && b_data(c,B_CHILD,45U)==egg
...
int main(int argc,char **argv) {
    ...
    clear_parties(c);b_create(c,QOL_PLAYER_PARTY,25U,0x123456F0U,0x11223344U);
    b_create(c,QOL_PLAYER_PARTY+100U,v->mother_species,0x34567801U,0x99887766U);
    ...
    unsigned initial_cycles=b_data(c,B_CHILD,32U);a_require(initial_cycles==v->cycles,"source-bound egg cycles changed");
    ...
    for(unsigned z=0;z<12288U && !b_hatched;++z) {
        b_step(c,read16(c,b_save1(c))==2U?QOL_KEY_RIGHT:QOL_KEY_LEFT);
    ...
    printf("\"child_species\":%u,\"child_level\":1,\"moves\":",v->child_species);a_array(v->child);printf(",\"pp\":");a_array(pp);
