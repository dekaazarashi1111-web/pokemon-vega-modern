# Pokémon Vega Modern — トーホク＋カントー二地方版 v1.4.0
...
2. BPS対応patcherで `vega-modern-kanto-v1.4.0.bps` をそのclean ROMへ直接適用する。
...
トーホクの既存遭遇表は上書きせず、外来生態設計293行をmap別の追加抽選として接続する。
通常の草むら77、洞窟・屋内31、水上23、いわくだき22に加え、朝昼／ずつき28、夜34、
夜間水上1、日替わり大量発生29、釣り21、水上／釣り兼用1、DexNav相当の隠れ枠7、
隠れ／タマゴ枠11、屋内異常8を隠れ遭遇として実装した。最初の草むらは5%で8種の候補から抽選する。
釣り枠は実際の釣り入口と使用中の竿を使い、バッジ数・殿堂入り前後のlevel条件も反映する。
設計上の個別依頼T-E04だけは専用quest eventを新設せず、2個目のバッジ＋いいつりざおへ
解禁条件を正規化している。「バッジ4個＋夜、または生態チケット」は、前半をそのまま保持し、
後半をせいたいレーダーの「夜固定」で代替する。

### せいたいレーダー

1個目のバッジ報酬として、だいじなもの「せいたいレーダー」（Item 348）を受け取る。
既存saveで未所持なら、シオウの技管理NPCが1個だけ補う。バッグから使うと現在modeを表示し、
RTC自動中は現在が「あさ・ひる」か「よる」かも同じ画面で確認できる。
次を選べる。

- `RTC自動`: emulatorの時計から朝昼／夜を選び、日付に応じた群れも自動抽選する。
- `朝昼固定` / `夜固定` / `群れ固定`: 時計に関係なく該当layerを確認できる。
- `隠れ探索`: 現在mapのDexNav／隠れ／屋内専用枠から、その場で1体を探す。

釣りはどのmodeでも釣り竿を使った時だけ判定する。RTC設定がない、時刻を待ちたくない、
特定layerを検証したい場合は手動modeを使えばよい。自動へ戻す操作も同じメニューにある。

## ポケモンの入手方法と取得イベント

コレクション対象1,206種と、到達性に必要な10フォームの全てに通常プレイ上の入手経路を用意した。
既存の野生・進化経路を維持し、不足分は201件の取得イベントへまとめている。カントー19マップに
いる既存NPC・固定シンボル24人へ話しかけると、解禁済み候補を5件ずつページ送りで選べる。
主な窓口はオーキド研究所（御三家・リージョン）、グレン研究所（化石復元）、ヤマブキの
ポケモンセンター2F（交換エミュレータ）、エスパーおやじの家（進化支援）、セキエイ高原の
記録官（旧世代・幻）で、フリーザー、サンダー、ミュウツー等は元の固定地点を使う。

- `CAPTURE`: 実際の固定野生戦を開始し、捕獲成功時だけ取得済みにする。撃破・逃走・電源断では
  再挑戦でき、捕獲済み個体の二重受取はできない。
- `GIFT` / `EGG`: 手持ちが満杯ならPCへ送る。手持ちとPCの両方が満杯なら何も変更せず断る。
  タマゴは受取時にclaimし、図鑑登録は実際に孵化した時だけ行う。
- `FOSSIL`: 対応する化石を所持している時だけグレン研究所で復元する。16経路と採掘補助を収録。
- `EVOLUTION_SUPPORT`: 現代進化条件を補助する。通信進化30経路はItem 395
  「リンクケーブル」で単独ROM進化でき、必要な持ち物条件も保持する。
- `TRADE_EMULATOR` / `SERVICE`: 通信相手を使わず交換判定・補助サービスを実行する。

解禁は新規開始、カントー早期到達、Vega殿堂入り、捕獲数に応じた調査rank、Kanto League後の
順で広がる。RTC待ちは必須にせず、時刻条件はせいたいレーダーの固定modeで再現できる。
取得履歴と中断中transactionは既存2 KiB台帳内の240 byte領域へsector 31保存される。
個体・手持ち・PC・図鑑は通常のゲーム内saveへ保存され、再起動後に両方を復元する。
...
v1.4.0で受付から実際に遊べるよう結合済みなのはTrialだけである。下表のStandard / Full /
Master、BP shop、施設外報酬遭遇は進行・content定義と回帰fixtureを保持しているが、実ROMの
受付・NPCへはまだ接続していない。

| tier | unlock | 主な形式・mechanic | v1.4.0 runtime |
|---|---|---|---|
...
post-v1.4.0 stage 31では、Trial完了chainにstage 30の49連勝特殊event key（claim bit 8）と、
Factory Master解禁後・Trial streak 100以上で一度だけ受け取れる非伝説色違い記念枠（claim bit 9）を
追加接続した。記念個体は137種poolからLv.50で生成し、party優先、満杯時PCへ送り、図鑑・取得台帳・
claimを通常save＋sector 31の復旧可能transactionで確定する。Master受付、region mix/Ultimate、
入場時gimmick選択そのものは未接続で、v1.4.0のtag／配布ROMは変更していない。
