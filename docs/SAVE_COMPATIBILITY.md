...
- v1.2.0 battery save: 対応。stage 20以後にserialized fieldを追加していないため、ゲーム内saveを
  v1.3.9でそのまま読み込める。先に旧版側でゲーム内saveを完了してからROMを切り替える。
...
- 1個目のバッジ報酬を通過済みでせいたいレーダーを持たない既存saveは、同じシオウNPCが
  Item 348を1個だけ補う。
- 旧版のemulator savestateは非互換。v1.3.9起動前にゲーム内saveへ戻す。

## v1.3.4から切り替える場合

提供された `vega-modern-kanto-v1.3.4-verified.srm` は通常のbattery saveとして検査でき、
v1.3.9でも利用できる。Delta環境やこのbattery saveが、表示Type・通知ループの原因ではない。
...
2. v1.3.9へ切り替え、savestateではなくゲーム内saveからRestartする。
