...
- v1.2.0 battery save: 対応。stage 20以後にserialized fieldを追加していないため、ゲーム内saveを
  v1.3.3でそのまま読み込める。先にv1.2.0側でゲーム内saveを完了してからROMを切り替える。
...
v1.3.3のFactory Trialは2,048-byte ledgerをCFRU-JPのsector 31 payloadへ直接確定する。
...
- v1.2.0以前のemulator savestateは非互換。v1.3.3起動前にゲーム内saveへ戻す。
