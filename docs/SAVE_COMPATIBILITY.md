...
- v1.2.0 battery save: 対応。stage 20以後にserialized fieldを追加していないため、ゲーム内saveを
  v1.3.8でそのまま読み込める。先に旧版側でゲーム内saveを完了してからROMを切り替える。
...
v1.3.8のFactory Trialは2,048-byte ledgerをCFRU-JPのsector 31 payloadへ直接確定する。
...
- 1個目のバッジ報酬を通過済みでせいたいレーダーを持たない既存saveは、同じシオウNPCが
  Item 348を1個だけ補う。
- 旧版のemulator savestateは非互換。v1.3.8起動前にゲーム内saveへ戻す。

## v1.3.4から切り替える場合

提供された `vega-modern-kanto-v1.3.4-verified.srm` は通常のbattery saveとして検査でき、
v1.3.8でも利用できる。Delta環境やこのbattery saveが、表示Type・通知ループの原因ではない。
v1.3.4では戦闘中の技画面でLを押すとFR由来の旧HELPが先に開き、CFRUの一時戦闘領域を
上書きするROM統合不具合があった。v1.3.8では戦闘中だけ旧HELPを抑止し、CFRU技詳細へLを渡す。

1. v1.3.4でゲーム内saveを作り、battery saveを別名でbackupする。
2. v1.3.8へ切り替え、savestateではなくゲーム内saveからRestartする。
