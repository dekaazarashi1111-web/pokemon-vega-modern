#include "mgba_pr16_circus_fairy_lock.h"
...
static void su_team(struct mCore *c)
{
    ...
    for(unsigned n=0;n<3U;++n){unsigned best=6U;uint64_t top=0U;
        for(unsigned i=0;i<6U;++i){uint64_t score=b25_rank(read16(c,0x0203DB20U),scores[i],types[i]);
            if(!(types[i]&~covered))score/=4U;
            if(!(used&(1U<<i)) && (best==6U || score>top)){best=i;top=score;}}
    ...
    unsigned legacy_plan[3],legacy_order[3],legacy_j=0U;
...
static void tp_consider(struct mCore *c)
{
    ...
    if(sr_hold(streak,read8(c,own+BATTLE_CORE_MON_TYPE1),read8(c,own+BATTLE_CORE_MON_TYPE2),
        read8(c,foe+BATTLE_CORE_MON_TYPE1),read8(c,foe+BATTLE_CORE_MON_TYPE2),attacks,
        ...
        unsigned deferred=hr_defer_seen(streak,1U,read16(c,own+BATTLE_CORE_MON_HP),read16(c,own+0x2CU));
    ...
    for(unsigned f=0;f<18000U;++f){
        ...
        unsigned same=read32(c,own+0x48U)==target_pid && read32(c,own+0x54U)==target_ot && read16(c,own)==species;
