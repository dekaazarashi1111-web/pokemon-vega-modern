...
static void sc_event(struct mCore *c,const char *label,unsigned battle){
    bp_require(c,sc_events++<800U,"Circus streak event budget exceeded");
...
static void sc_launch_battle(struct mCore *c,unsigned battle){
    ...
    sc_event(c,"confirmation",sc_base+battle);sc_frozen(c);cg_begin(c);
    for(unsigned f=0;f<18000U;++f){
    ...
    cg_end(c);sc_frozen(c);sc_event(c,"action",sc_base+battle);++sc_battles;
}
...
int ss_prefix_main(int argc,char **argv){
...
static void cg_end(struct mCore *c){(void)c;}
