...
static void bp_state(struct mCore *c, const char *label) {
    n_state(c,label);
    fprintf(stderr,"BP_CTRL label=%s frame=%u menu=%u stage=%u page=%u mode=%u status=%u window=%u cursor=%u count=%u snapshot=%u marker=%u bp=%u save=%u context_header_raw=%08x\n",label,b_frames,
...
int circus_historical_control(int argc,char **argv) {
